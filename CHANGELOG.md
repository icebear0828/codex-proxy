...
- Grok LB Claude shim on WSL (`127.0.0.1:8088`): isolated `data-grok-claude` profile with example configs, `scripts/grok-claude-server.mjs`, `start_grok_claude_proxy.sh`, and `install_grok_claude_systemd.sh` for user systemd after `codex-lb-go` pool `:2477`; `data-grok-claude/README.md` and `*.example` configs (`scripts/grok-claude-server.mjs`、`scripts/start_grok_claude_proxy.sh`、`scripts/install_grok_claude_systemd.sh`、`data-grok-claude/*.example`)

...
- Update build scripts and their extraction pattern config are tracked again so `npm run update` and the update path regression test do not fail from missing `scripts/build/{check-update,apply-update,full-update,types}.ts` or `config/extraction-patterns.yaml` files in clean checkouts.
- 同账号 `previous_response_not_found` / `unanswered_function_call` 恢复判定从 `proxy-handler.ts` 抽到独立 `proxy-retry-recovery.ts`，主 handler 只保留 affinity forget、恢复请求、清 `previous_response_id` / `turnState` 与 continue 的编排副作用；新增 helper 行为测试和 AST/import 边界测试防止错误分类与日志前缀处理回流到 orchestrator（`src/routes/shared/proxy-retry-recovery.ts`、`src/routes/shared/proxy-handler.ts`、`tests/unit/routes/shared/proxy-retry-recovery.test.ts`、`tests/unit/routes/shared/proxy-retry-recovery-boundary.test.ts`）

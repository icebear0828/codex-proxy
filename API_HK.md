...
`POST /v1/alpha/search` 支援由 ChatGPT OAuth 帳號池承載的普通 Codex 模型，並將請求轉發至 `/backend-api/codex/alpha/search`。當請求模型路由到 `wire=codex-responses` 的 API-key provider 時，代理亦支援下列全部非串流 JSON 端點：
